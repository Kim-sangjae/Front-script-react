...
export default function Profile(){
    // 파라미터로 받아온 username , 요청주소의 매개변수(방문한 프로필의 유저네임)
    const {username} = useParams();

    // 로그인 유저
    const {user,setUser} = useContext(AuthContext);

    // 로그인유저가 자신의 프로필을 방문했는지 여부 본인이 맞으면 true 아니면 false
    ...
    useEffect (()=>{
        ...
        // 여러개의 프로미스 객체를 처리하는 메서드 
        // 서버에 여러개의 데이터를 한번에 요청하기 위해 사용
        Promise.all([
            getProfile(username),
    ...
    function handleSignOut(){
        ...
        if(confirmed) {
            setUser(null); // authprovider 에 있는 메서드 
        }
    }
