...
export default function Carousel({photos}){
    ...
    return (
        <div className="overflow-x-hidden relative">
            ...
            <svg
            className={`w-4 cursor-pointer fill-white/[0.8] mx-2 ${index === 0 && 'hidden'}`}
            ...
            <svg
            className={`w-4 cursor-pointer fill-white/[0.8] mx-2 ${index === photos.length - 1 && 'hidden'}`}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
            onClick={() => setIndex(index + 1)}
            >
